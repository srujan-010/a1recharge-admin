...
// Method for Admin panel (unmasked)
kycSchema.methods.toAdminJSON = function toAdminJSON() {
  const getRaw = (v) => {
    if (!v) return null;
    return decrypt(v);
  };
  ...
  return {
    ...
    dob: this.dob,
    address: this.address,
    aadhaarNumber: getRaw(this.aadhaarNumber),
    panNumber: getRaw(this.panNumber),
    gstNumber: this.gstNumber,
    shopName: this.shopName,
    businessType: this.businessType,
    aadhaarFront: this.aadhaarFront,
