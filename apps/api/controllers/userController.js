...
const getNotificationPreferences = async (req, res, next) => {
  try {
    ...
    res.status(200).json({
      success: true,
      data: {
        notificationEnabled: req.user.notificationEnabled !== false,
        preferences
      }
    });
  } catch (error) {
...
const updateNotificationPreferences = async (req, res, next) => {
  try {
    const { notificationEnabled, preferences } = req.body;

    if (notificationEnabled !== undefined) {
      req.user.notificationEnabled = Boolean(notificationEnabled);
    }

    if (preferences && typeof preferences === 'object') {
      req.user.notificationPreferences = {
        rechargeNotifications: preferences.rechargeNotifications !== undefined ? Boolean(preferences.rechargeNotifications) : true,
        walletNotifications: preferences.walletNotifications !== undefined ? Boolean(preferences.walletNotifications) : true,
        promotionalNotifications: preferences.promotionalNotifications !== undefined ? Boolean(preferences.promotionalNotifications) : true,
        kycNotifications: preferences.kycNotifications !== undefined ? Boolean(preferences.kycNotifications) : true,
        systemNotifications: preferences.systemNotifications !== undefined ? Boolean(preferences.systemNotifications) : true,
        securityNotifications: true,
      };
    }
