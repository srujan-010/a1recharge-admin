...
const registerRetailer = async (req, res, next) => {
  try {
    ...
    // Trigger Welcome Notification with dynamic retailer name rendering
    notificationService.sendWelcomeNotification({ userId: user._id }).catch(err => {
      console.warn('[Auth] Welcome notification trigger warning:', err.message);
    });
