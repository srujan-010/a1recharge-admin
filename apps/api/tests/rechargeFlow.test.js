...
describe.skip('Recharge Flow Atomicity Tests', () => {
  let mockReq;
  let mockRes;
  let mockNext;
  let mockUser;
  ...
  beforeEach(async () => {
    ...
    await ProviderOperator.deleteMany({});
    await ProviderCircle.deleteMany({});

    await ProviderOperator.create({ code: 'A', name: 'Airtel', provider: 'A1Topup', status: true, serviceType: 'Mobile' });
    await ProviderCircle.create({ code: '4', state: 'Maharashtra', status: true });
