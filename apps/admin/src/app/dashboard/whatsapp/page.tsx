...
export default function WhatsAppDashboardPage() {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto animate-in fade-in duration-500">
      ...
      {/* Header Actions */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-gradient-to-r from-emerald-50 to-teal-50 dark:from-emerald-950/20 dark:to-teal-950/20 p-5 sm:p-6 rounded-[24px] border border-emerald-100 dark:border-emerald-900/30">
        <div className="flex items-center gap-3.5 sm:gap-4 min-w-0">
          <div className="w-12 h-12 rounded-2xl bg-emerald-500 text-white flex items-center justify-center shadow-lg shadow-emerald-500/20 shrink-0 self-center">
          ...
          <div className="flex flex-col justify-center min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              ...
              {isConnected ? (
                <Badge className="bg-emerald-500 hover:bg-emerald-600 text-white font-bold text-[10px] gap-1 px-2 py-0.5 shrink-0">
                  <CheckCircle2 className="w-3 h-3" /> Connected
                </Badge>
              ) : (
                <Badge variant="destructive" className="font-bold text-[10px] gap-1 px-2 py-0.5 shrink-0">
                  <AlertCircle className="w-3 h-3" /> Disconnected
                </Badge>
              )}
            </div>
            <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 mt-1 leading-snug break-words">
              Official WhatsApp Business Cloud API • Connected Number: {waba?.number || "+91 9975600499"}
            </p>
          </div>
        </div>

        <div className="flex items-center gap-2 w-full md:w-auto shrink-0 pt-2 md:pt-0 border-t md:border-t-0 border-emerald-200/40 dark:border-emerald-900/20">
          <Button
            variant="outline"
            onClick={() => refetch()}
            className="h-11 px-4 gap-2 font-bold text-xs rounded-xl border-slate-200 dark:border-slate-800 flex-1 md:flex-initial"
          >
          ...
          <Button
            ...
            disabled={isSyncing}
            className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold h-11 px-5 rounded-xl shadow-lg shadow-emerald-600/20 text-xs gap-2 flex-1 md:flex-initial"
          >
