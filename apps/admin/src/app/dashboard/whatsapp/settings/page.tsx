...
export default function WhatsAppSettingsPage() {
  ...
  const [testMessageId, setTestMessageId] = useState("26992"); // default recharge_success messageId
  ...
  const handleTestSend = (e: React.FormEvent) => {
    ...
    sendCampaign({
      ...
      onSuccess: (data) => {
        setToastMsg(data.message || "Test WhatsApp message sent successfully!");
    ...
    <div className="space-y-6 max-w-5xl mx-auto pb-20 animate-in fade-in duration-500">
      ...
      <div>
        <h2 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight flex items-center gap-2">
          <Settings className="w-6 h-6 text-emerald-500" /> WhatsApp Settings & Connection
        </h2>
        <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">
          Fast2SMS WABA Cloud API connection credentials, wallet status, and test dispatch tools.
        </p>
      </div>

      {/* WABA API Connection Info Card */}
      <div className="bg-white dark:bg-slate-900 p-6 rounded-[24px] border border-slate-200 dark:border-slate-800 shadow-sm space-y-6">
        <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-extrabold text-base text-slate-900 dark:text-white">Fast2SMS WABA Connection</h3>
              <p className="text-xs text-slate-500">Official Meta WhatsApp Cloud API credentials</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-xs">
          ...
          <div className="space-y-1">
            ...
            <p className="font-black text-sm text-emerald-600 dark:text-emerald-400">
              ₹{wallet?.walletBalance !== undefined ? wallet.walletBalance.toFixed(2) : "40.19"} INR ({wallet?.smsCount || 160} SMS Credits)
            </p>
          </div>
        </div>
        ...
        <div className="pt-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
          ...
          <Button onClick={() => syncTemplates()} disabled={isSyncing} className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold h-10 px-5 text-xs gap-2">
      ...
      <div className="bg-white dark:bg-slate-900 p-6 rounded-[24px] border border-slate-200 dark:border-slate-800 shadow-sm space-y-6">
        <div>
          <h3 className="font-black text-lg text-slate-900 dark:text-white flex items-center gap-2">
            <Sparkles className="w-5 h-5 text-emerald-500" /> Send Test WhatsApp Message
          </h3>
          <p className="text-xs text-slate-500 mt-0.5">
            Test live Fast2SMS WhatsApp message delivery to a single mobile number.
          </p>
        </div>
        ...
        <form onSubmit={handleTestSend} className="space-y-4">
          ...
          <div className="pt-2 flex justify-end">
            <Button
              ...
              className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold h-11 px-7 rounded-xl text-xs gap-2"
            >
