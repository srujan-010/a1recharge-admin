...
import { useRetailerProfile, useUpdateRetailerStatus } from "@/hooks/useRetailers";
...
import { 
  ...
  Minus, Send, RefreshCw, Key, Lock, Copy, Check, Search, Activity, Smartphone, 
  Sparkles, X, TrendingUp, IndianRupee, ArrowUpRight, ArrowDownRight, SmartphoneNfc, 
  MoreVertical, Edit, User, Fingerprint, Calendar
} from "lucide-react";
  ...
  return (
    <div className="space-y-6 max-w-[1600px] mx-auto pb-24 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-sm space-y-2">
          <div className="text-xs font-bold text-slate-500 uppercase tracking-wider flex justify-between">
            Monthly Recharge <Calendar className="w-4 h-4 text-blue-500" />
          </div>
          <div className="text-2xl font-black text-slate-900 dark:text-white font-mono">₹{((business.monthlyRechargePaise || 0) / 100).toFixed(2)}</div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-slate-900 rounded-3xl p-6 shadow-xl border border-slate-800 text-white relative overflow-hidden">
              ...
              <div className="grid grid-cols-2 gap-4 py-4 border-y border-slate-800/60 mb-6">
                ...
                <div className="space-y-3">
                  <div>
                    <p className="text-[10px] text-slate-500 uppercase font-bold">Today's Debit</p>
                    <p className="text-sm font-bold font-mono text-rose-400">-₹{((wallet.todaysDebit || 0) / 100).toFixed(2)}</p>
                  </div>
                  <div>
                    <p className="text-[10px] text-slate-500 uppercase font-bold">Lifetime Debit</p>
                    <p className="text-sm font-bold font-mono">-₹{((wallet.lifetimeDebit || 0) / 100).toFixed(2)}</p>
                  </div>
                </div>
              </div>
...
fs.writeFileSync(path.join(__dirname, '..', 'src', 'app', 'dashboard', 'retailers', '[id]', 'page.tsx'), pageContent.trim(), 'utf8');
